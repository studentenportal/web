...
class DocumentcategoryList(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        category_counts = (
            models.Document.objects.values("category", "dtype")
            .order_by()
            .annotate(count=Count("dtype"))
        )
        ...
        for c in categories:
            ...
            d["other"] = (
                counts[c.pk][models.Document.DTypes.SOFTWARE]
...
class DocumentFeed(Feed):
    ...
    def link(self, obj):
        return reverse(
            "documents:document_feed", kwargs={"category": slugify(obj.name)}
        )
    ...
    def items(self, obj):
        return models.Document.objects.filter(category=obj).order_by("-upload_date")[
            :10
        ]
...
class DocumentDownload(View):
    def get(self, request, *args, **kwargs):
        ...
        # Serve file
        filename = unicodedata.normalize("NFKD", doc.original_filename).encode(
            "ascii", "ignore"
        )
...
class DocumentThumbnail(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return sendfile(
            request, thumbnail_path, attachment=False, attachment_filename=filename
        )
...
class DocumentAdd(
    LoginRequiredMixin, DocumentAddEditMixin, DocumentcategoryMixin, CreateView
):
...
class DocumentEdit(
    LoginRequiredMixin, DocumentAddEditMixin, DocumentcategoryMixin, UpdateView
):
...
class DocumentDelete(LoginRequiredMixin, DocumentcategoryMixin, DeleteView):
    ...
    def get_success_url(self):
        """Redirect to documentcategory page."""
        messages.add_message(
            self.request, messages.SUCCESS, "Dokument wurde erfolgreich gelöscht."
        )
...
def document_rating(request, category, pk):
    ...
    context = {
        "doc": get_object_or_404(models.Document, pk=pk, category__name=category)
    }

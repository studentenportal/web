...
class DocumentCategory(models.Model):
    ...
    name = fields.CaseInsensitiveSlugField(
        "Kürzel", max_length=32, unique=True, help_text='z.B. "CompT1" oder "Prog3"'
    )
...
class Document(models.Model):
    ...
    description = models.CharField(
        "Beschreibung", blank=True, max_length=500, help_text="(Max. 500 Zeichen)"
    )
    url = models.URLField(
        "URL", null=True, blank=True, help_text="z.B. Link zu Github Repository"
    )
    ...
    document = models.FileField(
        "Datei", upload_to=document_file_name, help_text="(Max. 20MB)"
    )
    original_filename = models.CharField(
        "Originaler Dateiname", max_length=255, blank=True
    )
    ...
    license = models.PositiveSmallIntegerField(
        ...
        blank=True,
        help_text=mark_safe(
            'Lizenz, siehe <a href="http://creativecommons.org/choose/?lang=de">'
...
class DocumentRating(models.Model):
    ...
    document = models.ForeignKey(
        Document, related_name="DocumentRating", on_delete=models.CASCADE
    )

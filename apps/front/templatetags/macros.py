...
def do_macro(parser, token):
    ...
    for arg in args:
        if not r_valid_arg_name.match(arg):
            msg = "Argument '%s' to macro '%s' contains illegal characters. " + \
                  "Only alphanumeric characters and '_' are allowed."
            raise template.TemplateSyntaxError, msg % (arg, macro_name)
...
class UseMacroNode(template.Node):
    ...
    def render(self, context):
        for (arg, fe) in [(self.args[i], self.filter_expressions[i])

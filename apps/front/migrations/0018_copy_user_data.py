...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        "Move data from front.UserProfile and auth.User to front.User."
        db.execute('DELETE FROM front_user;');
        db.execute('''
            INSERT INTO front_user (
                id, password, last_login, is_superuser, username, first_name,
                last_name, email, is_staff, is_active, date_joined, twitter, flattr
            )
            SELECT
                auth_user.id, password, last_login, is_superuser, username,
                first_name, last_name, email, is_staff, is_active, date_joined,
                COALESCE(twitter, ''), COALESCE(flattr, '')
            FROM auth_user
                LEFT JOIN front_userprofile ON (front_userprofile.user_id = auth_user.id)
            ;''')
        db.execute('DELETE FROM auth_user;')
        db.execute('DELETE FROM front_userprofile;')


    def backwards(self, orm):
        "Move data from front.User to front.UserProfile and auth.User."
        # Delete old data
        db.execute('DELETE FROM front_userprofile;')
        db.execute('DELETE FROM auth_user;')
        # Copy data
        db.execute('''
            ...
            ;''')
        db.execute('''
            INSERT INTO front_userprofile (user_id, twitter, flattr)
            SELECT id, COALESCE(twitter, ''), COALESCE(flattr, '')
            FROM front_user
            ;''')
        # Replace front_user with dummy data
        db.execute('DELETE FROM front_user;')
